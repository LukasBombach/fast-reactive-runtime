...
export function createReactiveRuntime() {
  ...
  function value<V>(
    value: V,
    options?: { name: string }
  ): [get: () => V, set: (value: V) => void, reactions: Set<Effect>] {
  ...
  function computed<T = undefined>(
    fn: Computed<T>,
    options?: { name: string }
  ): [get: () => T | undefined, reactions: Set<Effect>] {
    const [get, set, reactions] = value<T | undefined>(undefined, options);
